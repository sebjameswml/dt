...
// In the absence of GNU i18n headers, define a dummy translation method: (Tam)
//char* _ (char* msg) { return msg; }
...
#ifdef _CUPS_LANGUAGE_PRIVATE_H_
	  ...
	  if (filetype == _CUPS_FILE_CHECK_DIRECTORY)
	    snprintf(message, sizeof(message),
		     ...
		     _cupsLangString(lang, _("Directory \"%s\" has insecure "
					     ...
					     "(0%o/uid=%d/gid=%d).")),
#else
                     "Directory \"%s\" has insecure permissions "
                     "(0%o/uid=%d/gid=%d).",
#endif
		     filename, fileinfo.st_mode, (int)fileinfo.st_uid,
		     (int)fileinfo.st_gid);
	    ...
	    snprintf(message, sizeof(message),
		     ...
		     _cupsLangString(lang, _("File \"%s\" has insecure "
					     ...
					     "(0%o/uid=%d/gid=%d).")),
#else
                     "File \"%s\" has insecure permissions "
                     "(0%o/uid=%d/gid=%d).",
#endif
		     filename, fileinfo.st_mode, (int)fileinfo.st_uid,
		     (int)fileinfo.st_gid);
...
#else
          "Directory \"%s\" contains a relative path.", filename);
#endif
	  else
	    snprintf(message, sizeof(message),
#ifdef _CUPS_LANGUAGE_PRIVATE_H_
		     _cupsLangString(lang, _("File \"%s\" contains a relative "
					     "path.")), filename);
#else
    "File \"%s\" contains a relative path.", filename);
#endif
          break;
    }
